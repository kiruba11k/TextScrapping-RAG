...
def get_rag_response(query):
    ...
    if retrieved_docs:
        ...
        return f"\n**RAG Retrieved Answer:** {response}"
    
    # If no RAG result, generate answer using LLM and label it
    llm = ChatGroq(model_name="Gemma2-9b-It")
    llm_response = llm.invoke(query)
    return f"\n**Generated by LLM (No relevant documents found):** {llm_response}"
...
workflow.add_node("llm_fallback", lambda query: f"\n**Generated by LLM (No relevant documents found):** {ChatGroq(model_name='Gemma2-9b-It').invoke(query)}")

workflow.set_entry_point("scraper")

# Define retrieval function with condition
def retrieval_condition(output):
    """Returns a dictionary with 'next' key deciding the path."""
    return {"next": "retriever" if output and "RAG Retrieved Answer" in output else "llm_fallback"}

# Add edges with conditional logic
workflow.add_edge("scraper", "retriever")
workflow.add_conditional_edges("retriever", retrieval_condition, {"retriever": "retriever", "llm_fallback": "llm_fallback"})
